...
- (void)popupView:(__kindof UIView *)targetView
    ...
    self.targetView.frame = CGRectMake(0, 0, targetView.width, targetView.height);
    self.targetView.centerX = [RTNavRouter fetchKeyWindow].centerX;

    CGFloat centY = [RTNavRouter fetchKeyWindow].centerY-targetView.height/2.0f;
    ...
    if (animationType == PopUpViewAnimationTypeCenter) {
        ...
        self.fromTransform = CGAffineTransformMakeTranslation(0, kScreenHeight+self.targetView.height);
        self.toTransfrom = CGAffineTransformMakeTranslation(0, kScreenHeight-self.targetView.height-XYPopupBottomHeight); //XYPopupBottomHeight适配iphoneX

        self.targetView.transform = CGAffineTransformMakeTranslation(0, kScreenHeight+self.targetView.height);
    }
    
    self.bgBtn.backgroundColor = UIColorWithRGB16RadixA(0x000000, ALPHA_NUM);
...
+ (BOOL)iPhoneNotchScreen {
    ...
    if ([UIDevice systemVersion] < 11.0f)  return false;
