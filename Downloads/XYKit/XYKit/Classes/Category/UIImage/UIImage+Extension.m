...
+ (UIImage *)yx_shotWithView:(UIView *)view scope:(CGRect)scope {
    CGImageRef imageRef = CGImageCreateWithImageInRect([self yx_shotWithView:view].CGImage, scope);
...
+ (NSData *)yx_resetSizeOfImageData:(UIImage *)source_image
    ...
    UIImage *newImage = [self yx_newSizeImage:defaultSize image:source_image];
     ...
     */
    // 思路：使用二分法搜索
    finallImageData = [self yx_halfFuntion:compressionQualityArr image:newImage sourceData:finallImageData maxSize:maxSize];
    ...
    while (finallImageData.length == 0) {
        ...
        UIImage *image = [self yx_newSizeImage:defaultSize
                                      image:[UIImage imageWithData:UIImageJPEGRepresentation(newImage,[[compressionQualityArr lastObject] floatValue])]];
        finallImageData = [self yx_halfFuntion:compressionQualityArr image:image sourceData:UIImageJPEGRepresentation(image,1.0) maxSize:maxSize];
    }
    return finallImageData;
}

// 调整图片分辨率/尺寸（等比例缩放）
+ (UIImage *)yx_newSizeImage:(CGSize)size
...
// 二分法
+ (NSData *)yx_halfFuntion:(NSArray *)arr
                  image:(UIImage *)image
    ...
    while(start <= end) {
        ...
        NSUInteger sizeOrigin = finallImageData.length;
        NSUInteger sizeOriginKB = sizeOrigin / 1024;
        DLog(@"当前降到的质量：%ld", (unsigned long)sizeOriginKB);
