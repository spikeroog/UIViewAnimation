...
+ (BOOL)isApplicationFirstLoad {
    ...
    NSString *lastRunVersion = [defaults objectForKey:FirstInstallOrUpdateStart];
    
    if (!lastRunVersion) {
        [defaults setObject:currentVersion forKey:FirstInstallOrUpdateStart];
        return YES;
...
+ (BOOL)isApplicationFirstInstallLoad {
    ...
    NSString *lastRunVersion = [defaults objectForKey:FirstInstallStart];
    
    if (!lastRunVersion) {
        [defaults setObject:currentVersion forKey:FirstInstallStart];
        return YES;
    } else {
        return NO;
    }
...
/**
 ...
 @param target 目标self 如：[RTNavRouter currentVC].self
...
+ (void)showCustomAlertViewWithTitle:(NSString *)title
    ...
    if (title) {
        ...
        [alertControllerTitleStr addAttribute:NSForegroundColorAttributeName value:UIColorWithRGB16Radix(0x222222) range:NSMakeRange(0, title.length)];
    ...
    if (message) {
        ...
        [alertControllerMessageStr addAttribute:NSForegroundColorAttributeName value:UIColorWithRGB16Radix(0x222222) range:NSMakeRange(1, message.length)];
    ...
    if (cancelMessage) {
        UIAlertAction *cancleBtn = [UIAlertAction actionWithTitle:cancelMessage style:UIAlertActionStyleCancel handler:
                                    cancelHandler];
        [cancleBtn setValue:UIColorWithRGB16Radix(0x9A9A9A) forKey:@"_titleTextColor"];
    ...
    if (sureMessage) {
        ...
        [sureBtn setValue:UIColorWithRGB16Radix(0xFFA801) forKey:@"_titleTextColor"];
    ...
    if (thridMessage) {
        ...
        [thridMessageBtn setValue:UIColorWithRGB16Radix(0xF2A801) forKey:@"_titleTextColor"];
...
/**
 ...
 BOOL resultType = [UtilityTools judgeTextMoneyTypeWithTextField:textField shouldChangeCharactersInRange:range replacementString:string isFirst:self.isFirstChar complete:^(BOOL isFirstChar) {
...
#pragma mark - 高效绘制圆角
/**
 简易版，使用CAShapeLayer和UIBezierPath，高效绘制圆角
 系统的`layer.cornerRadius`设置圆角会多次调用离屏渲染，十分消耗性能，会导致界面卡顿
 
 @param targetView 目标View
 */
+ (void)efficientDrawBead:(__kindof UIView *)targetView {
    // 串行队列
    dispatch_queue_t conCurrent_queue = dispatch_queue_create("drawCornerRadius", DISPATCH_QUEUE_SERIAL);
    // 主队列
    dispatch_queue_t main_queue = dispatch_get_main_queue();
    
    dispatch_async(conCurrent_queue, ^{
        // 绘制圆角
        CGFloat corner = (double)targetView.frame.size.width / 2;
        dispatch_async(main_queue, ^{
            UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:targetView.bounds byRoundingCorners:UIRectCornerAllCorners cornerRadii:CGSizeMake(corner, corner)];
...
/**
 高级版，使用CAShapeLayer和UIBezierPath，高效绘制圆角
...
+ (CGFloat)getLabelHeightWithText:(NSString *)text
    ...
    CGRect rect = [text boundingRectWithSize:CGSizeMake(width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:UIFontWithSize(fontSize)} context:nil];
...
+ (CGFloat)getWidthWithText:(NSString *)text
    ...
    CGRect rect = [text boundingRectWithSize:CGSizeMake(MAXFLOAT, height)
                                  ...
                                  attributes:@{NSFontAttributeName:UIFontWithSize(fontSize)}
                                     context:nil];
...
+ (NSString *)fetchAmountChineseStrWithNumber:(CGFloat)num {
    ...
    if ([formattedNumberString rangeOfString:@"点"].length > 0){
        ...
        DLog(@"---%@---长度%ld", lastStr, lastStr.length);
        if (lastStr.length >= 2) {
            // 在最后加上“分”
