...
// Tags label layout alignment, default is center.
@property (nonatomic, assign) TTGTagLabelAlignment labelAlignmentType;
...
// new api
@property (copy, nonatomic) NSString *arrowImageStr;
@property (copy, nonatomic) NSString *selectArrowImageStr;
...
- (void)cofigAlignment:(TTGTagLabelAlignment)type {
    if (type == TTGTagLabelAlignmentCenter) {
        _label.textAlignment = NSTextAlignmentCenter;
        
    } else if (type == TTGTagLabelAlignmentLeft) {
        _label.textAlignment = NSTextAlignmentLeft;
        
    } else if (type == TTGTagLabelAlignmentRight) {
        _label.textAlignment = NSTextAlignmentRight;
        
    }
}

- (void)configArrowImage:(NSString *)imageString {
    
    [self.arrowImage setImage:UIImageWithStr(imageString) forState:UIControlStateNormal];
}

- (void)commonInit {
    _label = [[GradientLabel alloc] init];
    
    if (self.labelAlignmentType == TTGTagLabelAlignmentCenter) {
        _label.textAlignment = NSTextAlignmentCenter;

    } else if (self.labelAlignmentType == TTGTagLabelAlignmentLeft) {
        _label.textAlignment = NSTextAlignmentLeft;

    } else if (self.labelAlignmentType == TTGTagLabelAlignmentRight) {
        _label.textAlignment = NSTextAlignmentRight;
    ...
    [self.arrowImage setImage:UIImageWithStr(@"checkbox_checked") forState:UIControlStateNormal];
    ...
    [self.arrowImage mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.offset(kAutoCs(22));
        make.height.offset(kAutoCs(22));
    }];
...
- (void)updateContentStyle {
    ...
    // 设置自定义右边按钮图片
    NSString *imgStr = _selected ? _config.selectArrowImageStr : _config.arrowImageStr;
    [self configArrowImage:imgStr];
    
    // 设置自定义label对齐方式
    [self cofigAlignment:_config.labelAlignmentType];
...
@property (nonatomic, assign, readwrite) NSInteger lastSelectedIndex;
// Tag selection limit, default is 0, means no limit 单选不可取消选中
...
#pragma mark - 单选还是多选（重复点击可取消选择），默认NO，单选
        // 单选还是多选（重复点击可取消选择），默认NO，多选（新增api，原框架不支持单选样式重复点击取消选择）
        if (!self.isSingleSelect) {
            // 上一次选中 有值，并且跟本次选中的 index 不同
            if (self.lastSelectedIndex != -1 && self.lastSelectedIndex != index) {
                // 取消上一次选中
                [self setTagAtIndex:self.lastSelectedIndex selected:NO];
            }
